...
uint16_t max_display_update_time = 0;
  ...
  void lcd_control_power_menu();
  void lcd_control_power_off_menu();
  void lcd_control_power_batt_menu();
...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void _lcd_user_gcode(const char * const cmd) {
      ...
      void lcd_user_gcode_5() { _lcd_user_gcode(PSTR(USER_GCODE_5 "Wylaczono")); }
    #endif
  ...
  void lcd_main_menu() {
    ...
    MENU_BACK(MSG_WATCH);
	  ...
	  MENU_ITEM(submenu, "Zasilanie", lcd_control_power_menu); // printospark dodane
  ...
  /********************************************
  * Printo Spark!
  * Dodajemy dodatkowe menu zasilania
  * Printo Spark!
  *********************************************/
  void lcd_control_power_menu() {
	  START_MENU();
	  ...
	  MENU_ITEM(submenu, "Wy³¹cz drukarkê", lcd_control_power_off_menu);
	  MENU_ITEM(submenu, "Bateria", lcd_control_power_batt_menu);

	  END_MENU();
  }
  void lcd_control_power_off_menu() {
	  START_MENU();
	  MENU_BACK("Power Menu");
  ...
  void lcd_control_power_batt_menu() {
	  int battery_raw;
	  battery_raw = analogRead(61);
	  float battery_voltage = battery_raw * (18 / 1024.0);
	  ...
	  START_MENU();
		  ...
		  STATIC_ITEM("Bateria [V]: ", false, false, ftostr32(battery_voltage));          // Print Count: 999
